        ...
        try {
            ...
            if ($route->items->isNotEmpty()) {
                ...
                $iaData = Cache::remember($cacheKey, now()->addHours(24), function () use ($route) {
                    ...
                    $apiKey = env('GEMINI_API_KEY');
                    //$url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={$apiKey}";
                    $url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?:generateContent?key={$apiKey}";
        ...
        try {
            ...
            if ($route->items->isNotEmpty()) {
                ...
                $iaData = Cache::remember($cacheKey, now()->addHours(24), function () use ($route) {
                    ...
                    // CORRECCIÓN: Se cambió "v1beta" por "v1" en la URL.
