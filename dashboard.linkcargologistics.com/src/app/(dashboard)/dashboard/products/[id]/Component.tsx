...
const CSRProductFormComponent: React.FC<any> = () => {
    ...
    formData
      ...
      .then((res: any) => {
        if (res?.product) {
          ...
          const extra = res.product.product || {};
          ...
          setInputs({
            ...
            brand: extra.brand || '',
            measure_unit: extra.measure_unit || '',
            measure_quantity: extra.measure_quantity || '',
            short_description: extra.short_description || '',
            long_description: extra.long_description || '',
            stock_control: !!extra.stock_control,
            stock_alert_level: extra.stock_alert_level || '',
            stock_reorder_amount: extra.stock_reorder_amount || '',
            model: extra.model || '',
            color: extra.color || '',
            sku: extra.sku || '',
            price: extra.price || '',
  ...
  const handleGalleryRemove = (i: number) => {
    setInputs((prev: any) => {
      const arr = Array.isArray(prev.gallery) ? prev.gallery : [];
      return { ...prev, gallery: JSON.stringify(arr.filter((_:any, idx:any) => idx !== i)) };
    });
  };
