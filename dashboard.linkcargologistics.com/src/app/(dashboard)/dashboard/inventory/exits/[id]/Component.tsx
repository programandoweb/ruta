  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <Card className="mb-4">
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          {isReadOnly ? (
            <>
              <div>
                <label className="text-sm text-navy-700 dark:text-white font-bold">Referencia</label>
                <div className="mt-1 text-sm">{inputs.reference}</div>
              </div>
              <div>
                <label className="text-sm text-navy-700 dark:text-white font-bold">Fecha de entrada</label>
                <div className="mt-1 text-sm">{inputs.movement_date}</div>
              </div>
              <div>
                <label className="text-sm text-navy-700 dark:text-white font-bold">Proveedor</label>
                <div className="mt-1 text-sm">
                  {providers.find(p => p.id == inputs.provider_id)?.name || '---'}
                </div>
              </div>
              <div>
                <label className="text-sm text-navy-700 dark:text-white font-bold">Notas</label>
            ...
            <>
              <InputField
                ...
                name="reference"
                id="reference"
                label="Referencia (factura u orden)"
                type="text"
              ...
              <SelectField
                ...
                id="provider_id"
                label="Proveedor"
                defaultValue={inputs.provider_id}
                setInputs={setInputs}
      ...
      {inputs.items?.length > 0 && (
        <Card className="mb-4">
          <div className="p-4">
            <h3 className="text-lg font-semibold mb-4">Productos en esta entrada</h3>
                  ...
                  return (
                    <tr key={idx}>
                      ...
                      <td className="p-2 text-right">
                        ${parseFloat(item.unit_cost || 0).toLocaleString('es-CO')}
                      </td>
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                ...
                <InputField
                  ...
                  type="number"
                  defaultValue={newItem.unit_cost}
                  setInputs={setNewItem}
                />
                <InputField
                  name="location"
