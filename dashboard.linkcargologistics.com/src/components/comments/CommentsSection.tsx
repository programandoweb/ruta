...
// Paleta de colores suaves por tipo
...
const CommentsSection: React.FC<CommentsSectionProps> = ({
  ...
  const types:any = Array.from(new Set(dataset.map((d) => d.type))).filter(Boolean);
  const filteredData =
    selectedType === 'Todos' ? dataset : dataset.filter((d) => d.type === selectedType);
      ...
      <div className="space-y-2">
        {filteredData?.map((c:any) => {
          ...
          return (
            <article
              key={c.id}
              className={`group relative rounded-md border p-3 shadow-sm transition-all ${colorClass}`}
            >
