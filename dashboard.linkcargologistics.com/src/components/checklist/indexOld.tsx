...
const ChecklistComponent = ({ steps, productionOrderId, onUpdateChecklist, onAddComment, completedSteps, commentsData }: ChecklistProps) => {
    ...
    // Actualizamos los pasos completados en el estado
    useEffect(() => {
        setCheckedSteps(completedSteps);
    }, [completedSteps]);

    // Inicializamos el estado de los comentarios desde commentsData
    useEffect(() => {
        const initialComments: { [key: number]: string } = {};
        steps.forEach((step) => {
            initialComments[step.step] = '';  // Inicializamos el input vacío
        });
        setComments(initialComments);  // Solo usamos el estado para el input actual, no para todos los comentarios previos.
    }, [steps]);
    ...
    const handleCheckboxChange = (step: number) => {
        ...
        // Enviar el checklist al backend
    ...
    const handleCommentChange = (step: number, comment: string) => {
        setComments((prevComments) => ({
            ...prevComments,
            [step]: comment,  // Actualizamos solo el comentario de este paso
        }));
    };
    ...
    const handleCommentSubmit = (step: number, event: React.KeyboardEvent<HTMLInputElement>) => {
        if (event.key === 'Enter' && comments[step]) {
            ...
            setComments((prev) => ({
                ...prev,
                [step]: '',  // Reseteamos el input del paso enviado
            }));
        }
    };
                        ...
                        <div key={index} className="col-span-1">
                            <div className="flex items-center space-x-3">
                                <input
                                    ...
                                    disabled={checkedSteps.includes(step.step)}
                                />
                                <label
