...
const title: string = "Parqueaderos"
const prefixed = 'parkings'
let formData: any

const defaultValues = { parkings_id: "", user_id: 0, status: 'activo', comentario: '' }
...
const AddParking: NextPage<Props> = ({ user_id }) => {
  ...
  useEffect(() => {
    const getInit = async () => {
      setLoading(true)
      formData.handleRequest(formData.backend + "/dashboard/dataset/parkings?user_id=" + user_id).then((response: any) => {
        if (response && response.userData && response.userData.parkings_all) {
          setDataset(response.userData.parkings_all)
        }
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault()
    formData.handleRequest(formData.backend + "/dashboard/dataset/set/parkings?user_id=" + user_id, "post", inputs).then((response: any) => {
      if (response && response.userData && response.userData.parkings_all) {
        setDataset(response.userData.parkings_all)
      }
    })
  ...
  const handleDeleteConfirm = () => {
    formData.handleRequest(formData.backend + "/dashboard/dataset/set/parkings?user_id=" + user_id, "put", {
        user_id: user_id,
        parkings_id:deleteItem.id,        
                  ...
                  <table className='w-full'>
                    ...
                    {dataset?.map((row: any, key: number) => (
                        <tr key={key} className='border-t text-xs'>                            
                            <td>    
                                <div>{row.label} <b>({row?.pivot?.status})</b></div>
                                {
              ...
              <motion.div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl text-center" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} transition={{ duration: 0.25 }}>
                ...
                <div className="p-4">
                  <div className="grid grid-cols-1  gap-4">
                    <SelectField required placeholder="Parqueadero" label="Parqueadero" defaultValue={inputs.parkings_id} setInputs={setInputs} options={parkings} name="parkings_id" variant="autenticación" />                    
                  </div>
                </div>
