...
const title: string = "Vehículos"
const prefixed = 'vehicles'
let formData: any

const defaultValues = { vehicle_id: "", user_id: 0, status: 'activo', comentario: '' }
...
const AddVehicle: NextPage<Props> = ({ user_id }) => {
  ...
  useEffect(() => {
    const getInit = async () => {
      setLoading(true)
      formData.handleRequest(formData.backend + "/dashboard/dataset/vehicles?user_id=" + user_id).then((response: any) => {
        if (response && response.userData && response.userData.vehicles_all) {
          setDataset(response.userData.vehicles_all)
        }
        if (response && response[prefixed]) {
          setparkings(response[prefixed])
        }     
        setLoading(false)   
      })
    }
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault()
    formData.handleRequest(formData.backend + "/dashboard/dataset/set/vehicles?user_id=" + user_id, "post", inputs).then((response: any) => {
      if (response && response.userData && response.userData.vehicles_all) {
        setDataset(response.userData.vehicles_all)
      }
    })
  ...
  const handleDeleteConfirm = () => {
    formData.handleRequest(formData.backend + "/dashboard/dataset/set/vehicles?user_id=" + user_id, "put", {
        user_id: user_id,
        vehicle_id:deleteItem.id,        
                  ...
                  <table className='w-full'>
                    ...
                    {dataset?.map((row: any, key: number) => (
                        <tr key={key} className='border-t text-xs'>                            
                            <td>    
                                <div>Tipo {row.type	} Placas {row.license_plate	}  <b>({row?.pivot?.status})</b></div>
                                {
                                    row?.pivot?.comentario!==''&&
                                    row?.pivot?.comentario!==null&&(
              ...
              <motion.div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl text-center" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} transition={{ duration: 0.25 }}>
                ...
                <div className="p-4">
                  <div className="grid grid-cols-1  gap-4">
                    <SelectField required placeholder="Parqueadero" label="Parqueadero" defaultValue={inputs.vehicle_id} setInputs={setInputs} options={parkings} name="vehicle_id" variant="autenticación" />                    
                  </div>
                </div>
